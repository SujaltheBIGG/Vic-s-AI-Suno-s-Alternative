...
function AppContent() {
  ...
  // Auth
  const { user, token, isAuthenticated, isLoading: authLoading, adoptSession, logout } = useAuth();
  ...
  // Adopt the session returned by sign-in / sign-up / Google.
  const handleAuthenticated = async (result: { user: any; token: string }) => {
    adoptSession(result);
