...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const defaultShareText = `🎵 "${song.title}" ${song.style ? `(${song.style})` : ''} - Made with Vic’s AI`;
  const twitterText = `🔥 Just created "${song.title}" with Vic’s AI - local AI music generation! ${song.style ? `#${song.style.replace(/\s+/g, '')}` : ''} #AIMusic #ACEStep`;
  const redditTitle = `[AI Music] ${song.title} - ${song.style || 'Original'} | Created with Vic’s AI`;
  const whatsAppText = `🎧 Listen to this AI-generated song!\n\n"${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `Genre: ${song.style}` : ''}\n\nMade with Vic’s AI - free and open source!`;
  const telegramText = `🎵 "${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `🎸 ${song.style}` : ''}\n\n🤖 Made with Vic’s AI`;
  const linkedInText = `Check out this AI-generated music: "${song.title}" - Created locally with Vic’s AI. #AIMusic #MusicTech #OpenSource`;
