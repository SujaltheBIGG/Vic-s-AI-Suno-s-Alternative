...
// ---- Single-port hosting: marketing site at '/', the Vic’s AI app at '/app' ----
const LANDING_DIR = path.join(__dirname, '../../landing');
const DIST_DIR = path.join(__dirname, '../../dist');

// The Vic’s AI app (built bundle) under /app, with SPA fallback
if (existsSync(DIST_DIR)) {
  // The built app pulls Tailwind, esm.sh and Google Fonts from CDNs.
  // Helmet's default script-src 'self' blocks those and the page renders unstyled.
  const appCsp = [
    "default-src 'self'",
    ...
    "connect-src 'self' blob: https://esm.sh https://cdn.tailwindcss.com https://huggingface.co https://*.hf.co",
    "worker-src 'self' blob:",
  ].join('; ');
...
// Marketing site at the root. Needs a relaxed CSP: the Webflow template
// pulls css/js/fonts from Webflow, Google and Cloudfront CDNs.
if (existsSync(LANDING_DIR)) {
  const landingCsp = [
    "default-src 'self'",
    "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.prod.website-files.com https://ajax.googleapis.com https://d3e54v103j8qbb.cloudfront.net https://fonts.googleapis.com",
  ...
  app.use((req, res, next) => {
    if (req.path === '/' || req.path.startsWith('/landing')) {
...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'Vic’s AI API' });
});
...
app.get('/song/:id', async (req, res) => {
  ...
  try {
    ...
    const description = `🎵 ${song.style} • Create your own AI music free on Vic’s AI`;
...
<head>
  ...
  <meta property="og:site_name" content="Vic’s AI">
...
<body>
  <p>Redirecting to <a href="${config.frontendUrl}?song=${song.id}">Vic’s AI</a>...</p>
</body>
</html>`);
  } catch (error) {
...
app.listen(config.port, '0.0.0.0', () => {
  console.log(`Vic’s AI Server running on http://localhost:${config.port}`);
  console.log(`Environment: ${config.nodeEnv}`);
  console.log(`Vic’s AI API: ${config.acestep.apiUrl}`);
