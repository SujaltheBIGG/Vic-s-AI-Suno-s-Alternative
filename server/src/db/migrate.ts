import { db, usingPostgres } from './pool.js';
...
function migrate(): void {
  ...
  try {
    ...
    db.exec(migrations);
    // Auth columns — SQLite lacks ADD COLUMN IF NOT EXISTS, so ignore dupes.
    for (const col of ['email TEXT', 'password_hash TEXT', 'google_id TEXT', 'avatar_from_google INTEGER DEFAULT 0']) {
...
// Run migrations for whichever engine is configured.
if (usingPostgres) {
