...
// Created lazily so that running on Postgres (DATABASE_URL set) never touches
// the filesystem — important on read-only or ephemeral hosts.
...
/** Proxy so existing `db.prepare(...)` / `db.exec(...)` calls still work. */
...
function executeQuery(sql: string, params?: unknown[], dbRef?: Database.Database): QueryResult {
  const target = dbRef ?? getDb();
  ...
  try {
    ...
    if (isSelect || convertedSql.includes('RETURNING')) {
      const stmt = target.prepare(convertedSql);
