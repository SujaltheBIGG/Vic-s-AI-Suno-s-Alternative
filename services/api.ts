...
export const authApi = {
  ...
  register: (email: string, password: string, username: string): Promise<AuthResponse> =>
    api('/api/auth/register', { method: 'POST', body: { email, password, username } }),

  login: (email: string, password: string): Promise<AuthResponse> =>
    api('/api/auth/login', { method: 'POST', body: { email, password } }),

  googleStatus: (): Promise<{ configured: boolean; redirectUri: string }> =>
    api('/api/auth/google/status'),

  /** Full-page redirect into Google's consent screen. */
  googleSignIn: (): void => { window.location.href = '/api/auth/google'; },
